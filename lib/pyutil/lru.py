...
Based on repoze.lru 0.6
What's news:
    2014-02-08 [LRUCache] add __getitem__ & __setitem__ & __contains__
...
class ExpiringLRUCache(object):
    ...
    def mset(self, kvs, timeout=None):
        for key, val in kvs.items():
            self.put(key, val, timeout=timeout)
...
class lru_cache_one(object):
    ...
    def __call__(self, f):
        ...
        marker = _MARKER
        def lru_cached(*arg):
            k = str(arg[0])                 #only here is different, just consider the first parameter
            val = cache.get(k, marker)
            if val is marker:
                val = f(*arg)
                cache.put(k, val)
